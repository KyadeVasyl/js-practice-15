...
function calculateCircleArea(radius) {
  ...
  const sOfCircle = Math.PI * Math.pow(radius, 2);
...
function sumPositiveNumbers(numbers) {
  ...
  let result = 0;
  //  по всіх числах у масиві за допомогою циклу for.
  for (let i = 0; i < numbers.length; i++) {
    let checkedNum = Math.sign(numbers[i]);
    if (checkedNum > 0) {
      result += numbers[i];
    }
  }
